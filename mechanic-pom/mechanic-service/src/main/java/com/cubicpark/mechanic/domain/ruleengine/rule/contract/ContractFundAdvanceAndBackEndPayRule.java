 ...
 * @since [产品/模块版本] （可选）
 */
public class ContractFundAdvanceAndBackEndPayRule extends MessageBranchNode {
    ...
    protected boolean makeDecision(Object paramObject) throws Exception {
        ...
        // 如果预付款比例非100%， 合同款项状态为正常回款未结清，判断款项类型是否为尾款 ，否则执行下一步
        if (!Constants.ADVANCE_RATIO_100.equals(advanceRatio) && Constants.FUND_STATE_BACKPAY.equals(fundState)
                && Constants.FUND_TYPE_ENDPAY.equals(contractFund.getFundType())) {
